...
export const AddressMap = () => {
	...
	const suggestView = useRef<any | null>(null)
	...
	const doSetCustomerAddress = (address: TAddress) => {
		dispatch(setCustomerAddress(address))
	}

	const mapClick = async (event: any) => {
		if (!ymaps.current) return false
		...
		if (opened) {
			...
			;(document.getElementById('searchAddress') as HTMLInputElement).value =
				addressLine
		}
	...
	const doSelect = async (event: any) => {
		if (!ymaps.current) return false
	...
	const onLoadMap = async (ympasInstance: YMapsApi) => {
		...
		const coords = [
			Number(customerAddress!.geo_lat),
			Number(customerAddress!.geo_lon),
		]
		const res = await ymaps.current.geocode(coords)
		const firstGeoObject = res.geoObjects.get(0)
		...
		const regions_arr = geojson.features.map((item: any) => ({
		...
		const currentMapData = {
			center:
				customerAddress?.geo_lat && customerAddress?.geo_lon
					? [Number(customerAddress.geo_lat), Number(customerAddress.geo_lon)]
					: [55.751574, 37.573856],
			zoom,
		}
